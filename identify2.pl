...
sub choose_start_point {
   my @list;
   foreach (@{$fetch_array}) {
      push(@list,sprintf("%-32s: %s-%d-%d-%d",
                         $_->{file_name},$_->{program_name},$_->{series_number},$_->{episode_number},$_->{section_number}));
   }
...
sub fetch_new_files {
   ...
   for $fn (sort {expand($a) cmp expand($b) }<$dir/V*.mp4> )
   {
      ...
      $video_length=sprintf("%02d:%02d:%02d.%003d",$vhours,$vmins,$info->{SS},$info->{MS});
